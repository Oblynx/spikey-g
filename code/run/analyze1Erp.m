...
parameters.feature.wave= struct( ...
  ...
  'waveFrq',[0.5,30], ...       % Transform frequency range
  'voicesPerOct',16, ...
  'padmode','zpd', ...
...
parameters.class.svm= struct( ...
  ...
  'svmPlotGraphs',false, ...              % Plot various descriptive graphs
	'singlePredictorPerformance',true, ... % Assess each predictor independently
	'singlePredictorPerformThreshold',40 ...% Show result only if it exceeds this threshold
);
parameters.gen= struct( ...
  'verbose',1, ...                          % 0= just errors, 1= info, 2= +parameters
  'erpTimeExtension',0.2, ...                   % Extend ERP time duration by this amount
